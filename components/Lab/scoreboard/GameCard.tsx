...
export default function GameCard({ game, lineScore, gradient }: GameCardProps) {
  ...
  return (
    ...
    >
      <TeamsScoreRow>
        ...
        <CenterInning>
          <InningArrow aria-hidden="true">{getInningArrow(lineScore.inningHalf)}</InningArrow>
          <span>
        ...
        <LinescoreTable>
          ...
          <tbody>
            {(['away', 'home'] as const).map((side) => (
              <tr key={side}>
                <TeamHeading scope="row">{game.teams[side].team.name.split(' ').pop()}</TeamHeading>
                {Array.from({ length: totalInnings }, (_, i) => (
                  <td key={i}>{lineScore.innings[i]?.[side]?.runs ?? ''}</td>
                ))}
                <RunsCell>{lineScore.teams[side].runs}</RunsCell>
...
const ScoreboardCard = styled.div<{ $gradient: string }>`
...
const Team = styled.div<{ $align?: 'right' }>`
  flex: 1 1 0;
  min-width: 0;
  text-align: ${({ $align }) => $align ?? 'left'};
`

const CenterInning = styled.div`
  display: flex;
  flex: 0 0 auto;
...
const TeamHeading = styled.th`
  font-weight: 600;
`

const RunsCell = styled.td`
  font-weight: 600;
`
